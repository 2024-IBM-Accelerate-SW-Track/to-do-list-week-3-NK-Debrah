...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      let color = "#ffffffff"; // Default to white
      const dueDate = new Date(todo.dueDate);
      const currentDate = new Date();

      if (dueDate < currentDate) {
        color = "#ffcccc"; // Light red for overdue items
      }
      ...
      return (
        <Grid key={todo.id}>
          <Card style={{ marginTop: 10, background: color }} data-testid={todo.content}>
            ...
            current date/time {todo.dueDate}. Also, the item's id is utilized in order to correctly delete an item from the Todo list */}
